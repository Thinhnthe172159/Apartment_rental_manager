                    ...
                    <div class="content-wrapper pb-0">
                        ...
                        <div class="row">
                            <div class="col-12 grid-margin stretch-card">
                                <div class="card">
                                    <div class="card-body">
                                        <h4 class="card-title">Maintenance Report</h4>
                                        <!--<p class="card-description">Basic form elements</p>-->
                                        <form action="LandLordDashboardEditMaintenanceReport" method="post" class="forms-sample">
                                            <input type="hidden" name="reportId" value="${report.id}">
                                            <p style="size: 30px"><strong>Tenant:</strong> ${report.tenantId.first_name} ${report.tenantId.last_name}</p>
                                            <p><strong>Apartment:</strong> ${report.apartmentId.name}</p>
                                            <p><strong>Status:</strong> 
                                                <c:choose>
                                                    <c:when test="${report.status == '1'}">Pending</c:when>
                                                    <c:when test="${report.status == '2'}">In-progress</c:when>
                                                    <c:when test="${report.status == '3'}">Done</c:when>
                                                    <c:when test="${report.status == '4'}">Postponed</c:when>
                                                    <c:otherwise>Unknown</c:otherwise>
                                                </c:choose>
                                            </p>
                                            <p><strong>Date Posted:</strong> ${report.datePost}</p>
                                            <p><strong>Context:</strong> ${report.context}</p>
                                            <p><strong>Images:</strong></p>
                                            <div class="row">
                                                <div class="col-md-4">
                                                    <img style="width: 100%; border-radius: 5px" src="uploads/${report.image1}" alt=""/>
                                                </div>
                                                <div class="col-md-4">
                                                    <img style="width: 100%; border-radius: 5px" src="uploads/${report.image2}" alt=""/>
                                                </div>
                                                <div class="col-md-4">
                                                    <img style="width: 100%; border-radius: 5px" src="uploads/${report.image3}" alt=""/>
                                                </div>
                                            </div>&nbsp;
                                            <div>
                                                <c:choose>
                                                    <c:when test="${report.status == '1'}">
                                                        <button class="btn btn-primary" type="submit" name="newStatus" value="2">Start Progress</button>
                                                    </c:when>
                                                    <c:when test="${report.status == '2'}">
                                                        <button class="btn btn-success" type="submit" name="newStatus" value="3">Complete</button>
                                                        <button class="btn btn-warning" type="submit" name="newStatus" value="4">Postpone</button>
                                                    </c:when>
                                                    <c:when test="${report.status == '4'}">
                                                        <button class="btn btn-primary" type="submit" name="newStatus" value="2">Resume Progress</button>
                                                    </c:when>
                                                </c:choose>
                                            </div>
                                        </form>
