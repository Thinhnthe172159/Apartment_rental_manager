 ...
 * @author thinh
 */
@WebServlet(name = "UpdateApartmentPost4", urlPatterns = {"/UpdateApartmentPost4"})
public class UpdateApartmentPost4 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UpdateApartmentPost4</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UpdateApartmentPost4 at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("Cập Nhật".equals(submit)) {
            ap.setPost_status(4); // trạng thái lưu nháp bài đăng nhưng vẫn là hết hạn
    ...
    private int processPayment(Apartment_Post ap, Payment_method paymentMethod, int week, User user, HttpSession session, HttpServletRequest request) {
        ...
        if ((user.getMoney()) < newPayment) {
            ap.setPost_status(4);
            session.setAttribute("message", "b");// trạng thái không đủ tiền đăng bài thì nó sẽ yêu cầu chúng ta cần phải nạp thêm tiền vào tài khoản
            return 0;
        } else {
            ...
            session.setAttribute("message", "a"); // trạng thái sau khi chúng ta đã thanh toán bài đăng trong ví thành công
